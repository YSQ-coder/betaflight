...
static const void *cmsx_SetFreedom(displayPort_t *pDisplay, const void *ptr)
{
    ...
    saveConfigAndNotify();
    cmsMenuExit(pDisplay, (void *)CMS_POPUP_SAVEREBOOT);
    return NULL;
}

static const void *cmsx_SetMgpPro(displayPort_t *pDisplay, const void *ptr)
{
    UNUSED(ptr);
    setSpec(SPEC_MGP_PRO);
    saveConfigAndNotify();
    cmsMenuExit(pDisplay, (void *)CMS_POPUP_SAVEREBOOT);
    return NULL;
}
...
static const OSD_Entry cmsx_menuSpecEntries[] =
{
    ...
    { specArray[SPEC_MGP_PRO].name, OME_Funcall, cmsx_SetMgpPro, NULL },
